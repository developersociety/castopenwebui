...
<ConfirmDialog
	...
	on:confirm={() => {
		deleteCharityHandler(selectedCharity.id);
	}}
/>
	...
	<div class="mt-0.5 mb-2 gap-1 flex flex-col md:flex-row justify-between">
		...
		<div class="flex gap-1">
			<div class=" flex w-full space-x-2">
				...
				<div>
					<Tooltip content={$i18n.t('Add Charity')}>
						<button
							class=" p-2 rounded-xl hover:bg-gray-100 dark:bg-gray-900 dark:hover:bg-gray-850 transition font-medium text-sm flex items-center space-x-1"
							on:click={() => {
								showAddCharityModal = !showAddCharityModal;
							}}
						>
			...
			>
				<tr class="">
					...
					<th
						scope="col"
						class="px-3 py-1.5 cursor-pointer select-none"
						on:click={() => setSortKey('charity_id')}
					>
						<div class="flex gap-1.5 items-center">
							{$i18n.t('Charity ID')}

							{#if orderBy === 'charity_id'}
								<span class="font-normal"
									>{#if direction === 'asc'}
										<ChevronUp className="size-2" />
									{:else}
					...
					<th
						scope="col"
						class="px-3 py-1.5 cursor-pointer select-none"
						on:click={() => setSortKey('is_imported')}
					>
						<div class="flex gap-1.5 items-center">
							{$i18n.t('Is imported?')}
			...
			<tbody class="">
				{#each charities as charity}
					<tr class="bg-white dark:bg-gray-900 dark:border-gray-850 text-xs">
						...
						<td class=" px-3 py-1"> {charity.charity_id ?? ''} </td>
						...
						<td class=" px-3 py-1"> {charity.is_imported} </td>

						<td class="px-3 py-1 text-right">
							<div class="flex justify-end w-full">
								<Tooltip content={$i18n.t('Edit Charity')}>
									<button
